    ...
    if (($_POST['csrf_token'] ?? '') !== $_SESSION['csrf_token']) {
        ...
        if (empty($selected_ids)) {
            ...
            switch ($_POST['bulk_action']) {
                case 'activate':
                    $stmt = $conn->prepare("UPDATE sub_categories SET status = 1 WHERE id IN ($placeholders)");
                ...
                case 'deactivate':
                    $stmt = $conn->prepare("UPDATE sub_categories SET status = 0 WHERE id IN ($placeholders)");
                    $stmt->bind_param($types, ...$selected_ids);
                    $stmt->execute();
                ...
                case 'delete':
                    $stmt = $conn->prepare("DELETE FROM sub_categories WHERE id IN ($placeholders)");
                    $stmt->bind_param($types, ...$selected_ids);
                    $stmt->execute();
...
<head>
    ...
    <title>Medical Department Management | Admin Dashboard</title>
                    ...
                    <div class="white_card card_height_100 mb_30">
                        <div class="card-header bg-white border-0 py-3">
                            <div class="d-flex justify-content-between align-items-center flex-wrap">
                                <div>
                                    <h2 class="mb-0 fw-bold">Medical Department Management</h2>
                                    <p class="text-muted mb-0 small">Manage medical specialities shown on the public department directory</p>
                                </div>
                                <div class="mt-2 mt-sm-0 d-flex align-items-center gap-2">
                                    ...
                                    <a href="add-sub-category.php" class="btn_1">
                                        <i class="fas fa-plus me-2"></i>Add New Sub Department
                                    </a>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="QA_table mb_30">
                                        <div class="table-responsive">
                                            <table class="table lms_table_active table-bordered table-hover">
                                                ...
                                                <tbody>
                                                    <?php echo get_sub_category_admin_rows(); ?>
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Bulk selection — operates on checkboxes currently in the DOM. DataTables
        // only keeps the active page's rows in the DOM, so this is a per-page
        // "select all", same as the Service Category Management page.
        ...
        // Rows get swapped in/out by DataTables on page/search changes, but the
        // <tbody> element itself persists, so the delegated 'change' listener
        // above keeps working — just refresh the toolbar's counts each redraw.
