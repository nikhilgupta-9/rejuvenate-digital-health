...
$schools_res = mysqli_query($conn, "SELECT id, school_name, school_uid FROM schools ORDER BY school_name ASC");

$error = '';
$old = $_POST ?: ['type' => $default_type];

if ($school_id) {
    $sstmt = $conn->prepare("SELECT * FROM schools WHERE id=?");
    $sstmt->bind_param('i', $school_id); $sstmt->execute();
    $school = $sstmt->get_result()->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$school_id) throw new Exception("Please select a school.");
        ...
        $sstmt = $conn->prepare("SELECT id FROM schools WHERE id=?");
        $sstmt->bind_param('i', $school_id); $sstmt->execute();
        if ($sstmt->get_result()->num_rows === 0) throw new Exception("Selected school does not exist.");

        if ($email) {
            $chk = $conn->prepare("SELECT id FROM school_members WHERE email=? AND school_id=?");
            $chk->bind_param('si', $email, $school_id); $chk->execute();
            if ($chk->get_result()->num_rows > 0) throw new Exception("A member with this email already exists in this school.");
        }
        ...
        if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
            ...
            $filename = 'member_' . time() . '_' . mt_rand(100,999) . '.' . $ext;
        ...
        $password_hash = $password ? password_hash($password, PASSWORD_BCRYPT, ['cost' => 12]) : null;
        $dob_val = $dob ?: null;

        $ins = $conn->prepare("INSERT INTO school_members
            (school_id, type, name, email, phone, dob, gender, blood_group, aadhar_number, address, profile_pic,
             class, section, roll_number, admission_number, employee_id, designation, assigned_class, password, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $ins->bind_param('isssssssssssssssssss',
            $school_id, $type, $name, $email, $phone, $dob_val, $gender, $blood_group, $aadhar_last4, $address, $profile_pic,
            $class, $section, $roll_number, $admission_number, $employee_id, $designation, $assigned_class, $password_hash, $status
        );
        if (!$ins->execute()) throw new Exception("Failed to add member: " . $conn->error);

        $_SESSION['success_message'] = ucfirst($type) . " \"$name\" added successfully.";
        header("Location: school-view.php?id=$school_id&tab=members");
        exit();
    } catch (Exception $e) {
        $error = $e->getMessage();
        $old = $_POST;
...
<head>
    ...
    <title>Admin | Add School Member</title>
                    ...
                    <div class="col-12">
                        <div class="page-header mb-4">
                            <div class="d-flex align-items-center justify-content-between">
                                <h2 class="mb-0">Add School Member</h2>
                                <a href="<?= $school_id ? 'school-view.php?id='.$school_id : 'school-members.php' ?>" class="btn btn-outline-secondary">
                            ...
                            <form method="post" enctype="multipart/form-data" id="memberForm">
                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-select" name="school_id" required>
                                            ...
                                            <?php mysqli_data_seek($schools_res, 0); while ($sc = mysqli_fetch_assoc($schools_res)): ?>
                                                <option value="<?= $sc['id'] ?>" <?= $school_id == $sc['id'] ? 'selected' : '' ?>>
                                                    <?= htmlspecialchars($sc['school_name']) ?> (<?= htmlspecialchars($sc['school_uid']) ?>)
                                                </option>
                                            <?php endwhile; ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-12 mb-2">
                                        ...
                                        <input type="hidden" name="type" id="typeInput" value="<?= htmlspecialchars($old['type'] ?? 'Student') ?>">
                                        <div class="row g-2">
                                            <div class="col-4">
                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="name" required value="<?= htmlspecialchars($old['name'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="phone" maxlength="10" inputmode="numeric" value="<?= htmlspecialchars($old['phone'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="blood_group" placeholder="e.g. O+" value="<?= htmlspecialchars($old['blood_group'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <label class="form-label">Aadhaar (last 4 digits only)</label>
                                        <input type="text" class="form-control" name="aadhar_last4" maxlength="4" inputmode="numeric" placeholder="XXXX" value="<?= htmlspecialchars($old['aadhar_last4'] ?? '') ?>">
                                        <small class="text-muted">For compliance, only the last 4 digits are stored.</small>
                                    </div>
                                    <div class="col-md-4 mb-3">
                                ...
                                <div class="row mb-4" id="studentFields">
                                    ...
                                    <div class="col-md-3 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="class" placeholder="e.g. 10" value="<?= htmlspecialchars($old['class'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-3 mb-3">
                                        <label class="form-label">Section</label>
                                        <input type="text" class="form-control" name="section" placeholder="e.g. A" value="<?= htmlspecialchars($old['section'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-3 mb-3">
                                        <label class="form-label">Roll Number</label>
                                        <input type="text" class="form-control" name="roll_number" value="<?= htmlspecialchars($old['roll_number'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-3 mb-3">
                                        <label class="form-label">Admission Number</label>
                                        <input type="text" class="form-control" name="admission_number" value="<?= htmlspecialchars($old['admission_number'] ?? '') ?>">
                                    </div>
                                </div>
                                ...
                                <div class="row mb-4" id="staffFields">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="employee_id" value="<?= htmlspecialchars($old['employee_id'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <label class="form-label">Designation</label>
                                        <input type="text" class="form-control" name="designation" placeholder="e.g. Math Teacher, Principal" value="<?= htmlspecialchars($old['designation'] ?? '') ?>">
                                    </div>
                                    <div class="col-md-4 mb-3" id="assignedClassField">
                                        <label class="form-label">Assigned Class</label>
                                        <input type="text" class="form-control" name="assigned_class" placeholder="e.g. 10-A" value="<?= htmlspecialchars($old['assigned_class'] ?? '') ?>">
                                    </div>
                                </div>

                                <div class="row mb-4">
                                    <div class="col-12"><h4 class="section-title">Login Access (Optional)</h4></div>
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label">Password</label>
                                        <input type="password" class="form-control" name="password" minlength="6" placeholder="Leave blank for no portal login">
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-12 mt-2">
                                        <button type="submit" class="btn btn-primary me-2"><i class="fas fa-save me-2"></i> Add Member</button>
                                        <a href="<?= $school_id ? 'school-view.php?id='.$school_id : 'school-members.php' ?>" class="btn btn-outline-secondary"><i class="fas fa-times me-2"></i> Cancel</a>
                                    </div>
                                </div>
                            </form>
                        </div>
