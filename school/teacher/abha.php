...
$hp_stmt->bind_param('i', $teacher_id);
$hp_stmt->execute();
...
$pend_stmt->bind_param('i', $teacher_id);
$pend_stmt->execute();
...
$hist_stmt->bind_param('i', $teacher_id);
$hist_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'request_link') {
        if ($teacher['abha_linked']) {
            ...
            if (strlen($raw) !== 14) {
                ...
                $ins->bind_param('iiss', $teacher_id, $teacher_school_id, $abha_fmt, $abha_addr);
                if ($ins->execute()) {
    ...
    if ($action === 'cancel_request' && $pending_req) {
        ...
        $del->bind_param('ii', $pending_req['id'], $teacher_id);
        if ($del->execute()) {
...
<head>
  ...
  <style>
    ...
    .abha-card {
      background: var(--ab-green); border-radius:16px; color:#fff;
      padding:24px 22px; position:relative; overflow:hidden; margin-bottom:20px;
    }
    .abha-card::before { content:''; position:absolute; top:-30px; right:-30px; width:160px; height:160px; background:rgba(255,255,255,.07); border-radius:50%; }
    .abha-card::after  { content:''; position:absolute; bottom:-50px; left:-20px; width:200px; height:200px; background:rgba(255,255,255,.05); border-radius:50%; }
    .abha-num  { font-size:1.35rem; font-family:monospace; font-weight:700; letter-spacing:.1em; margin:8px 0 4px; }
    .abha-addr { font-size:.82rem; opacity:.8; }
    .abha-lbl  { font-size:.6rem; opacity:.65; text-transform:uppercase; letter-spacing:.1em; }
    .abha-verified-badge { display:inline-flex; align-items:center; gap:5px; background:rgba(255,255,255,.2); border:1px solid rgba(255,255,255,.3); border-radius:20px; padding:3px 10px; font-size:.7rem; font-weight:700; }

    .abha-empty { background:#f0fdf4; border:2px dashed #86efac; border-radius:16px; padding:30px 22px; text-align:center; margin-bottom:20px; }

    .info-row { display:flex; align-items:center; justify-content:space-between; padding:8px 0; border-bottom:1px solid #f3f4f6; }
    ...
    .req-form-card { background:#fff; border-radius:12px; padding:22px; box-shadow:0 1px 8px rgba(0,0,0,.06); margin-bottom:20px; border-top:3px solid var(--ab-green); }
    ...
    .wiz-tabs button.active { background:var(--ab-green); color:#fff; border-color:var(--ab-green); }
    ...
    .step-ind .cur { color:var(--ab-green); font-weight:700; }
    .abha-ok-card { background:var(--ab-green); border-radius:14px; color:#fff; padding:18px; text-align:center; margin-bottom:12px; }
    .abha-ok-card .num { font-family:monospace; font-size:1.2rem; font-weight:700; letter-spacing:.1em; margin:6px 0 2px; }
    .abha-preview-mini { background:var(--ab-green); border-radius:10px; color:#fff; padding:12px 16px; margin-bottom:12px; font-size:.8rem; }
...
<body>
<?php $active_page = 'abha'; $base_path = ''; include '../inc/sidebar-teacher.php'; ?>

<div class="school-topbar">
  <div class="d-flex align-items-center gap-2">
    <button class="sidebar-toggler" id="sidebarToggle"><i class="fas fa-bars"></i></button>
    <div>
      <div style="font-size:1rem;font-weight:600;color:#1f2937;"><i class="fas fa-id-card me-2" style="color:#00875a;"></i>My ABHA</div>
      <div style="font-size:.75rem;color:#6b7280;">Ayushman Bharat Health Account</div>
    </div>
  </div>
  <div class="d-flex align-items-center gap-2">
    <a href="dashboard.php" class="btn btn-sm btn-outline-secondary"><i class="fas fa-arrow-left me-1"></i><span class="d-none d-sm-inline">Dashboard</span></a>
...
<main class="school-content">
  <div class="row justify-content-center">
    <div class="col-lg-7">
      ...
      <?php if ($teacher['abha_linked'] && $teacher['abha_id']): ?>
      <!-- ═══ ABHA LINKED ═══ -->
      <div class="abha-card">
        <div style="position:relative;z-index:1;">
          <div class="abha-lbl">Ayushman Bharat Health Account</div>
          <div class="abha-num"><?= htmlspecialchars($teacher['abha_id']) ?></div>
          <?php if ($teacher['abha_address']): ?>
            <div class="abha-addr"><i class="fas fa-at me-1" style="font-size:.7rem;"></i><?= htmlspecialchars($teacher['abha_address']) ?></div>
          <?php endif; ?>
          <div class="mt-3 d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div>
              <div style="font-size:.75rem;font-weight:600;"><?= htmlspecialchars($teacher['name']) ?></div>
              <div style="font-size:.68rem;opacity:.75;"><?= htmlspecialchars($teacher_uid) ?></div>
            </div>
            <?php if (!empty($teacher['abha_verified'])): ?>
          ...
          <?php if ($teacher['abha_linked_at']): ?>
            <div style="font-size:.63rem;opacity:.6;margin-top:10px;">
              <i class="fas fa-calendar me-1"></i>Linked on <?= date('d M Y', strtotime($teacher['abha_linked_at'])) ?>
            </div>
          <?php endif; ?>
        </div>
      </div>
      ...
      <div class="sec-card">
        <div class="sec-card-head">
          <div class="icon" style="background:#f0fdf4;color:#00875a;"><i class="fas fa-id-card"></i></div>
          <div><h6>ABHA Details</h6><p>Your linked health account information</p></div>
        </div>
        <div class="sec-card-body">
          <div class="info-row">
            <span class="info-label">ABHA Number</span>
            <span class="info-val" style="font-family:monospace;color:#00875a;"><?= htmlspecialchars($teacher['abha_id']) ?></span>
          </div>
          <div class="info-row">
            <span class="info-label">ABHA Address</span>
            <span class="info-val"><?= $teacher['abha_address'] ? htmlspecialchars($teacher['abha_address']) : '—' ?></span>
          </div>
          <div class="info-row">
          ...
          <div class="info-row">
            ...
            <span class="info-val"><?= date('d M Y', strtotime($teacher['abha_linked_at'])) ?></span>
          </div>
          <?php endif; ?>
        </div>
      </div>
        ...
        <div id="tabLink">
          <div id="stepL1">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="link_abha_in" placeholder="XX-XXXX-XXXX-XXXX" maxlength="19" oninput="fmtAbha(this,'prev_num')">
          ...
          <div id="stepL2" style="display:none;">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control otp-big" id="link_otp_in" placeholder="• • • • • •" maxlength="6" inputmode="numeric">
            </div>
        ...
        <div id="tabCreate" style="display:none;">
          <div id="stepC1">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="create_aadhaar" placeholder="XXXX XXXX XXXX" maxlength="14" inputmode="numeric"
          ...
          <div id="stepC2" style="display:none;">
            ...
            <div class="mb-3">
              <input type="text" class="form-control otp-big" id="create_aadhaar_otp" placeholder="• • • • • •" maxlength="6" inputmode="numeric">
            </div>
            <div class="d-flex gap-2">
          ...
          <div id="stepC2b" style="display:none;">
            <p class="step-ind"><span class="cur">Step 2b</span> — Mobile number</p>
            <div class="mb-3">
        ...
        <div id="tabFind" style="display:none;">
          ...
          <div id="stepF1">
            ...
            <p style="font-size:.78rem;color:#4b5563;margin-bottom:10px;">
              Search ABDM registry using your registered Mobile Number or Aadhaar Number to find existing ABHA ID.
            </p>
              ...
              <div class="mb-3">
                ...
                <div class="input-group">
                  ...
                  <input type="text" class="form-control" id="find_mobile_in"
                    ...
                    value="<?= htmlspecialchars(substr(preg_replace('/\D/', '', $teacher['phone'] ?? ''), -10)) ?>">
                </div>
      ...
      <?php if ($pending_req): ?>
      <div class="sec-card" style="border-left:4px solid #d97706;">
        <div class="sec-card-body">
          <div class="d-flex align-items-center gap-2 mb-3">
            <i class="fas fa-hourglass-half text-warning fa-lg"></i>
            <div>
              <div style="font-weight:700;color:#92400e;font-size:.9rem;">Link Request Pending</div>
              <div style="font-size:.75rem;color:#6b7280;">Waiting for school admin approval</div>
            </div>
          </div>
      ...
      <div class="req-form-card">
        ...
        <p style="font-size:.78rem;color:#6b7280;margin-bottom:14px;">Submit your ABHA number and the school admin will link it to your account.</p>
        <div class="abha-preview-mini mb-3">
          <div class="pnum" id="prev_num">XX-XXXX-XXXX-XXXX</div>
        ...
        <form method="POST">
          ...
          <div class="mb-3">
            <label class="form-label fw-semibold" style="font-size:.82rem;">ABHA Address <small class="fw-normal text-muted">(optional)</small></label>
            <div class="input-group">
      ...
      <?php endif; /* abha_linked */ ?>

      <!-- Health Summary -->
      <?php if ($hp): ?>
      <div class="sec-card">
        <div class="sec-card-head">
          <div class="icon" style="background:#fff0f0;color:#dc2626;"><i class="fas fa-heartbeat"></i></div>
          <div><h6>Health Summary</h6><p>Your health profile data</p></div>
        </div>
        <div class="sec-card-body">
          ...
          <?php if ($hp['known_allergies']): ?>
            <div class="info-row"><span class="info-label"><i class="fas fa-allergies me-1 text-warning"></i>Allergies</span><span class="info-val"><?= htmlspecialchars($hp['known_allergies']) ?></span></div>
          <?php endif; ?>
          <?php if ($hp['chronic_conditions']): ?>
            <div class="info-row"><span class="info-label"><i class="fas fa-notes-medical me-1 text-danger"></i>Conditions</span><span class="info-val"><?= htmlspecialchars($hp['chronic_conditions']) ?></span></div>
          <?php endif; ?>
        </div>
      </div>
      <?php else: ?>
      <div class="sec-card">
        <div class="sec-card-body text-center py-4" style="color:#9ca3af;">
          <i class="fas fa-heartbeat fa-2x mb-2" style="opacity:.25;"></i>
          <p class="mb-0" style="font-size:.83rem;">Health profile not yet set up. Contact your school admin.</p>
        </div>
      </div>
      <?php endif; ?>
      ...
      <div class="sec-card">
        <div class="sec-card-head">
          <div class="icon" style="background:#f3f4f6;color:#6b7280;"><i class="fas fa-history"></i></div>
          <div><h6>Request History</h6><p>Your previous ABHA link requests</p></div>
        </div>
      ...
      <!-- About ABHA -->
      <div class="sec-card" style="border-left:4px solid #00875a;">
        <div class="sec-card-body">
          <h6 class="fw-bold mb-2"><i class="fas fa-info-circle me-2 text-success"></i>What is ABHA?</h6>
          <p style="font-size:.78rem;color:#4b5563;line-height:1.6;margin-bottom:10px;">
            <strong>Ayushman Bharat Health Account (ABHA)</strong> is a 14-digit unique digital health ID under India's Ayushman Bharat Digital Mission (ABDM).
          </p>
          <ul style="font-size:.75rem;color:#6b7280;line-height:1.8;padding-left:18px;margin-bottom:10px;">
            <li>Links all your health records across hospitals &amp; clinics</li>
            <li>Enables paperless, consent-based health data sharing</li>
            <li>Free to create at <a href="https://healthid.ndhm.gov.in/" target="_blank" rel="noopener">healthid.ndhm.gov.in</a></li>
          </ul>
        </div>
      </div>
...
function setLoader(show, msg = '') {
  ...
  const lm = document.getElementById('wLoaderMsg');
...
async function initLink() {
  ...
  const res = await abdmPost('init_link', {abha_id: document.getElementById('link_abha_in').value, auth_method: authMethod});
  if (res.success) {
    document.getElementById('linkOtpMsg').textContent = `OTP sent to ${authMethod === 'AADHAAR_OTP' ? 'your Aadhaar mobile' : 'your registered mobile'}. Valid 10 min.`;
...
async function verifyAadhaarOtp() {
  ...
  if (res.success) {
    if (!res.mobileLinked) { showCreate('C2b'); }
    else {
...
async function genLinkedMobileOtp() {
  ...
  if (res.success) { showCreate('C2c'); } else { wAlert(res.message); }
}
async function verifyLinkedMobileOtp() {
  ...
  if (res.success) { showCreate('C3'); } else { wAlert(res.message); }
}
async function createAbha() {
  ...
  if (res.success) {
    document.getElementById('success_abha_num').textContent  = res.abha_id      || '';
    document.getElementById('success_abha_addr').textContent = res.abha_address || '';
    ['C1','C2','C2b','C2c','C3'].forEach(s => { const el = document.getElementById('step'+s); if (el) el.style.display = 'none'; });
    document.getElementById('stepSuccess').style.display = 'block';
  } else { wAlert(res.message); }
}
switchTab('link');
