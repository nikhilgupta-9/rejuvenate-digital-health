...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .s-body { max-width: 600px; margin: 0 auto; padding: 18px 14px 90px; }

    /* Hero */
    .profile-hero {
      background: var(--primary); border-radius: 16px; color: #fff;
      padding: 22px 20px; margin-bottom: 18px;
      display: flex; align-items: center; gap: 16px;
      position: relative; overflow: hidden;
    }
    .profile-hero::after { content:''; position:absolute; right:-30px; top:-30px; width:140px; height:140px; background:rgba(255,255,255,.06); border-radius:50%; }
    .p-avatar { width:58px; height:58px; border-radius:15px; background:rgba(255,255,255,.2); display:flex; align-items:center; justify-content:center; font-size:1.4rem; font-weight:700; flex-shrink:0; position:relative; z-index:1; }

    /* Section cards */
    .s-card { background:#fff; border-radius:12px; padding:16px 18px; margin-bottom:14px; box-shadow:0 1px 6px rgba(0,0,0,.07); }
    .s-card-title { font-size:.7rem; font-weight:700; text-transform:uppercase; letter-spacing:.8px; color:#6b7280; display:flex; align-items:center; gap:7px; margin-bottom:14px; padding-bottom:10px; border-bottom:1px solid #f3f4f6; }
    ...
    .info-row .lbl { font-size:.74rem; color:#6b7280; }
    .info-row .val { font-size:.82rem; font-weight:600; color:#1f2937; text-align:right; }
...
<div class="s-body">
  ...
  <div class="profile-hero">
    ...
    <div style="position:relative;z-index:1;flex:1;min-width:0;">
      ...
      <div style="font-size:.7rem;opacity:.65;margin-top:3px;font-family:monospace;"><?= htmlspecialchars($student_uid) ?></div>
    </div>
    <div style="position:relative;z-index:1;flex-shrink:0;">
      <?php if ($student['abha_linked']): ?>
        <span style="background:rgba(255,255,255,.2);border-radius:20px;padding:4px 10px;font-size:.68rem;font-weight:700;white-space:nowrap;">
  ...
  <div class="s-card">
    ...
    <div class="info-row"><span class="lbl"><i class="fas fa-school me-1"></i>School</span><span class="val"><?= htmlspecialchars($student_school) ?></span></div>
    ...
    <div class="info-row"><span class="lbl"><i class="fas fa-hashtag me-1"></i>Roll No.</span><span class="val"><?= htmlspecialchars($student['roll_number'] ?? '—') ?></span></div>
    <div class="info-row"><span class="lbl"><i class="fas fa-fingerprint me-1"></i>Member UID</span><span class="val" style="font-family:monospace;font-size:.74rem;"><?= htmlspecialchars($student_uid) ?></span></div>
    <div class="info-row"><span class="lbl"><i class="fas fa-clock me-1"></i>Last Login</span><span class="val"><?= $student['last_login'] ? date('d M Y, h:i A', strtotime($student['last_login'])) : 'First login' ?></span></div>
  </div>
      ...
      <div class="mb-3">
        ...
        <div class="input-group">
          ...
          <input type="email" class="form-control" name="email"
            value="<?= htmlspecialchars($student['email'] ?? '') ?>" placeholder="you@example.com">
        </div>
      </div>
      <div class="mb-3">
  ...
  <div class="s-card" style="border-left:4px solid var(--primary);">
    ...
    <div class="info-row">
      ...
      <?php else: ?>
        <span style="color:#9ca3af;font-size:.78rem;"><i class="fas fa-times-circle me-1"></i>Not set (login via OTP)</span>
      <?php endif; ?>
    </div>
    <div class="info-row">
    ...
    <div class="info-row">
      ...
      <span style="color:#16a34a;font-size:.78rem;font-weight:600;"><i class="fas fa-circle me-1" style="font-size:.5rem;vertical-align:middle;"></i>Active</span>
    </div>
  </div>
