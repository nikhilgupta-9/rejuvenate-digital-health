...
| 33d | `migration_whatsapp_automation.sql` | `whatsapp_message_log` (polymorphic entity_id, no FK, matches abdm_audit_logs pattern) + `whatsapp_templates` (event_type → Meta template name/lang/param_order, only `otp_verification` seeded) + `whatsapp_optouts` (STOP/UNSUBSCRIBE) + `appointments.reminder_24h_sent`. Backs `lib/WhatsAppNotifier.php` — the event-driven dispatcher on top of the existing OTP-only `lib/WhatsAppOtp.php`. See `WHATSAPP_AUTO_BRIEF.md`. |
| 35 | `migration_school_subscription_referral.sql` | **`school_plans`** (SaaS subscription tiers — Free/Growth/Scale seeded, admin-editable, separate from the per-student `school_health_plans`), **`school_subscriptions`** (one row per request; *every* plan needs admin approval — pending_payment → pending_approval → active/rejected/expired, unlike the doctor flow which auto-activates on payment), `schools.referred_by` (FK to another school, mirrors `doctors.referred_by`), **`school_referral_earnings`** (25% commission ledger on a referred school's first paid+approved subscription; payout tracked via `payout_status` — manual/offline, mirrors `doctor_referral_earnings` + `admin/settlements.php`). Backs `admin/school-subscription-plans.php`, `admin/school-subscriptions.php`, `admin/school-referrals.php`, `school/subscription.php`, `school/referrals.php`. |
| 36 | `migration_parent_consent_secure_link.sql` | `school_members.parent_mobile` (registered parent/guardian mobile — distinct from `school_members.phone`, the student's own), `parent_consent_forms.verified_via` ENUM('token','manual') — 'token' rows arrived through an HMAC-signed per-student link (`lib/ConsentToken.php`) that set `member_id` directly at submit time, closing the IDOR gap where a generic school-wide link relied on a post-hoc fuzzy name match (`doctor/inc/consent-helper.php`) to ever link a submission to a real student. Backs the token/generic mode split in `school/parent-consent.php` and the "Send Consent Link" action in `school/members/list.php` → `school/send-consent.php`. |
| 37 | `migration_parent_consent_identity.sql` | Phase 3 — parent identity strengthening. `parent_consent_forms.identity_check` ENUM('not_applicable','matched','mismatched') — token-mode-only comparison of the parent's declared Aadhaar-linked mobile against `school_members.parent_mobile`; a mismatch never blocks submission, just flags the row for `admin/parent-consents.php` review. `mobile_otp_verified` / `mobile_otp_verified_at` — `parent_mobile` is now WhatsApp-OTP verified before submit, reusing `util/otp-service.php` / `registration_otps` (new role `parent_consent`, added to `OTP_ALLOWED_ROLES`) via the shared `util/otp-widget.php` component — no new OTP infra. |
