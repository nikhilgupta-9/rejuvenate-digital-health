...
<style>
.info-section{background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,.05);padding:20px 22px;margin-bottom:16px;max-width:640px;}
.section-title{font-size:.9rem;font-weight:700;color:#374151;margin-bottom:6px;}
.section-title i{color:#dc3545;margin-right:6px;}
.warn-box{background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:14px 16px;font-size:.84rem;color:#7f1d1d;margin-bottom:16px;}
...
.hist-row{display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #f3f4f6;font-size:.82rem;}
...
<main class="doctor-content">
    ...
    <?php else: ?>
      <form method="POST">
        <div class="form-group">
          ...
          <textarea name="reason" class="form-control" rows="3" placeholder="Let us know why you're leaving..."></textarea>
        </div>
        <div class="form-group">
          <label>Confirm Password <span class="text-danger">*</span></label>
          <input type="password" name="password" class="form-control" required placeholder="Enter your current password">
        </div>
        <button type="submit" class="btn btn-danger" onclick="return confirm('Are you sure you want to request account deletion?');">
  ...
  <div class="info-section">
    <div class="section-title" style="color:#374151;"><i class="fa fa-history" style="color:#0C74C5;"></i> Request History</div>
    <?php foreach ($requests as $r): ?>
      <div class="hist-row">
        <div>
          Requested <?= date('d M Y', strtotime($r['requested_at'])) ?>
          <?php if (!empty($r['reason'])): ?><div class="text-muted" style="font-size:.76rem;"><?= htmlspecialchars($r['reason']) ?></div><?php endif; ?>
        </div>
        <div>
